...
void DemoPortal::sendUserData()
{
    ...
    else if(mapIdToCommandType[Id] == "List")
    {
        ...
        listing.push_back(product(dataRecieved[2], dataRecieved[3], 
                                  std::stof(dataRecieved[4]), stoi((dataRecieved[5]))));
    }
...
vector <product> DemoPortal::sortPrevList(vector <product> &listing)
{
...
// In the sortedList.
void DemoPortal::sendUserList(vector <product> &sortedList)
{
